 	...
 	page_position	: function(e,moveP,$self){ 		
        ...
        if(car2._touchDeltaY<=0){
 		...
 		function init_next(node){
 			...
 			car2._page.not('.action').addClass('f-hide').removeClass('z-animate');
	...
	init : function(){
		...
		$(window).on('load',function(){
			...
			setTimeout(function(){
				...
				// 显示正面
				//$('#j-mengban').removeClass('f-hide').addClass('z-animate');;

				// 显示封面内容
				setTimeout(function(){
					$('.translate-back').removeClass('f-hide');
					$('#j-start').removeClass('f-hide');
					setTimeout(function() {
						$('#j-start').addClass('z-animate');
					}, 10);
